...
ContinuousBatchingPipeline::ContinuousBatchingForSpeculativeDecodingImpl::get_generated_requests() {
    ...
    for (const auto& request : m_requests) {
        ...
        for (const auto& sequence : request->get_running_sequences()) {
            ...
            generated_request.insert({{sequence_id, { sequence->get_generated_ids(), sequence->get_generated_log_probs(), num_processed_tokens, sequence->get_hidden_state() } }});
        }
    }
    return result;
}
...
ContinuousBatchingPipeline::ContinuousBatchingForSpeculativeDecodingImpl::update_request(uint64_t request_id,
    ...
    for (auto& request : m_requests) {
        ...
        if (!m_is_validation_mode_enabled && !current_finished_prefill) {
    ...
    while (to_generate) {
        ...
        for (auto& request : m_requests) {
            ...
            if (!sampling_params.is_assisting_generation()) {
                ...
                // During the prompt processing phase, we can pause the generation of this request to avoid `draft
                // model` getting too far ahead of `main model`. This is a safety measure to prevent `draft model` from
